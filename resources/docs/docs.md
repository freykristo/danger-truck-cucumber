...
The `home-routes` handler in the `danger-truck-cucumber.routes.home` namespace
...
The `home-page` function will in turn call the `danger-truck-cucumber.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `danger-truck-cucumber.handler` namespace:
...
Request middleware functions are located under the `danger-truck-cucumber.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `danger-truck-cucumber.dev-middleware` namespace found in
